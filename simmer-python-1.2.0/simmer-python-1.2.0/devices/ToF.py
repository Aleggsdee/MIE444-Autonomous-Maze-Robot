'''
Defines a SimMeR device representing an ToF VL5310x  sensor.
'''
...
class ToF(Device):
    ...
    def __init__(self, info: dict):
        ...
        # Device height
        self.height = info.get('height', CONFIG.block_size) # TODO
        ...
        self.visible_measurement_time = info.get('visible_measurement_time', 0.05)    # Measurement time on screen (s) # TODO
        self.visible_measurement_buffer = 0

        # Simulation parameters
        self.beamwidth = info.get('beamwidth', 15)              # Beamwidth of the ToF sensor
        self.num_rays = info.get('num_rays', 13)                 # Number of rays to test
        self.min_range = info.get('min_range', 1.1)               # Minimum range in inches
        self.max_range = info.get('min_range', 78.7402)             # Maximum range in inches
        self.error_pct = info.get('error', 0.03)                # Percent error (0-1)
    ...
    def _define_rays(self):
        '''Define the rays used to get the ToF distance.'''
    ...
    def draw_measurement(self, canvas):
        '''Draw ToF sensor rays on the canvas'''
    ...
    def simulate(self, value: float, environment: dict):
        '''
        Simulates the performance of an ToF sensor.
    ...
    def _block_visible(self, BLOCK):
        '''Determines whether the block is visibile to an ToF sensor based on its height.'''
