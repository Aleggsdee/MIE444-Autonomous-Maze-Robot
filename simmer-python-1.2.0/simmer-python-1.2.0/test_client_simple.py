...
while RUN_DEAD_RECKONING:
    ...
    distance_u0 = sensor_distances['u0']
    distance_u1 = sensor_distances['u1']
    distance_u2 = sensor_distances['u2']
    
    if distance_u0 > FRONT_THRESHOLD and distance_u1 > SIDE_MIN_THRESHOLD and distance_u2 > SIDE_MIN_THRESHOLD:
        ...
        if distance_u1 <= SIDE_MIN_THRESHOLD and FRONT_THRESHOLD != BIG_FRONT_THRESHOLD:
            print('moving left')
            transmit(packetize('xx'))
            [responses, time_rx] = receive()
            ...
            [responses, time_rx] = receive()
        elif distance_u2 <= SIDE_MIN_THRESHOLD and FRONT_THRESHOLD != BIG_FRONT_THRESHOLD:
            print('moving right')
            transmit(packetize('xx'))
            [responses, time_rx] = receive()
            transmit(packetize('d0:-0.1'))
